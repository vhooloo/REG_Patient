...
class REG_PatientViewTopList extends ViewList {
	...
	function display(){
        ...
        $select_refill = ' , DATE_FORMAT(tab3.next_rx_refill_due_c,"%m/%d/%Y") refill ';
		...
		if ($_POST['mysort'] == 'week') 
		{
			...
			$testwhere  .= $where_refill . ' AND tab3.next_rx_refill_due_c BETWEEN NOW() AND DATE_ADD(NOW(), INTERVAL 60 DAY) '; 
		...
		$testquery = $testselect . $testfrom . $testwhere;
