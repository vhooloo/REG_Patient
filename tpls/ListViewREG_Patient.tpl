...
<body class='default'>
<table width="1112px" ><tr><td>
<div id ="test" height='340px'><h3  >Calendar</h3>
<div><iframe frameborder="0" scrolling="no" width="95%" height='340px' id="iframe" onload='javascript:resizeIframe(this);' src ="index.php?entryPoint=calender&module=REG_Patient"></iframe></div></div></td></tr></table>
...
function nWindow (w,h,patient_id)
{
...
NWin = window.open("index.php?entryPoint=calender&module=REG_Patient&parent_id="+patient_id, "", "toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width="+w+",height="+h+",top=" + top + ", left=" + left);
NWin.focus();
setTimeout(function() { document.getElementById('NWin').focus(); }, 5);

NWin.onbeforeunload = function(){document.getElementById('iframe').contentWindow.location.reload(true);}
...
{/literal}
	...
	var linkrenderer = function (row, column, value) {
			...
			//	return '<select id="mysort'+row+'" name="mysort'+row+'" onchange="if (document.getElementById(\'mysort'+row+'\').selectedIndex ==1 ) loadUrl(\'./index.php?module=REG_Patient&action=PrescriptionRefill&record='+value+'\'); else if(document.getElementById(\'mysort'+row+'\').selectedIndex ==2 ) loadUrl(\'./index.php?module=REG_Patient&action=PatientEncounter&record='+value+'\'); else if(document.getElementById(\'mysort'+row+'\').selectedIndex ==3 )  loadUrl(\'./index.php?module=REG_Patient&action=treatmentplan&record='+value+'\'); else if(document.getElementById(\'mysort'+row+'\').selectedIndex ==4 ) nWindow(\'1000\',\'400\',\''+value+'\'); "> <option value="Action"  selected>Action</option><option value="Refill" >Refill</option><option value="Enc" >Encounter</option><option value="Tp" >TreatmentPlan</option>	<option value="CM" >Create Meeting</option>	</select>';
				return '<select id="mysort'+row+'" name="mysort'+row+'" onchange="switch (document.getElementById(\'mysort'+row+'\').selectedIndex) { case 1: loadUrl(\'./index.php?module=REG_Patient&action=PrescriptionRefill&record='+value+'\'); break; case 2:  loadUrl(\'./index.php?module=REG_Patient&action=PatientEncounter&record='+value+'\'); break; case 3: loadUrl(\'./index.php?module=REG_Patient&action=treatmentplan&record='+value+'\');break;case 4: loadUrl(\'./index.php?module=REG_Patient&action=riskevaluation&patid='+value+'\');break; case 5: nWindow(\'1100\',\'500\',\''+value+'\'); break;}"> <option value="Action"  selected>Action</option><option value="Refill" >Refill</option><option value="Enc" >Encounter</option><option value="Tp" >Treatment Plan</option><option value="risk" >Risk Evaluation</option>	<option value="CM" >Create Meeting</option></select>';
    }
 ...
 <script type='text/javascript'>$(function(){$('#test').accordion({ collapsible: true , active: false } );  }); </script>;
	{/literal}
