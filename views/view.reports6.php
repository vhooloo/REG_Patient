...
( SELECT DATE_FORMAT(tab4.next_pill_ct_c,'%m/%d/%Y') ref1 FROM reg_encounter_cstm tab4 where tab4.id_c = enclink )  next_pill_count, ( SELECT risklvl_c risk1 FROM reg_encounter_cstm tab3 where tab3.id_c = enclink )  risk FROM reg_patient p LEFT JOIN reg_patient_cstm rec ON  rec.id_c=p.id WHERE p.id in (SELECT distinct pe.reg_patient_reg_encounterreg_patient_ida FROM `reg_patient_reg_encounter_c` pe, reg_encounter_cstm e WHERE pe.reg_patient_reg_encounterreg_encounter_idb=e.id_c AND (next_pill_ct_c is not null))";
